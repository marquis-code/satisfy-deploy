<template>
    <div class="min-h-screen">
          ...
          <div class="relative z-10">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
              <div class="flex items-center">
                <div class="bg-gradient-to-br from-orange-400 to-orange-600 p-3 rounded-lg shadow-lg mr-4 transform transition-transform hover:scale-105">
              ...
              <button 
                ...
                class="group bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white px-5 py-2.5 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 flex items-center"
              >
                <span class="relative">
                  <PlusIcon class="w-5 h-5 mr-2 transition-transform duration-300 group-hover:rotate-90" />
                </span>
                <span>Add Category</span>
                <span class="absolute inset-0 rounded-lg overflow-hidden">
          ...
          <div class="flex flex-col md:flex-row justify-between mb-6 gap-4">
            ...
            <div class="relative">
              ...
              <input 
                v-model="searchQuery" 
                type="text" 
                class="pl-10 pr-4 py-2 w-full md:w-64 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-200"
                    ...
                    >
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div class="flex space-x-3">
                          ...
                          >
                            <PencilIcon class="w-5 h-5 text-orange-500 group-hover:text-orange-600 transition-colors duration-200" />
                          ...
                          >
                            <TrashIcon class="w-5 h-5 text-red-500 group-hover:text-red-600 transition-colors duration-200" />
                            <span class="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap">
                              Delete
                            </span>
                          </button>
          ...
          <div class="flex min-h-screen items-center justify-center p-4">
            ...
            >
              ...
              <div class="mb-6">
                ...
                <div class="relative">
                  <input 
                    ...
                    type="text" 
                    class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-200"
              ...
              <div class="flex justify-end space-x-3">
                <button 
                  @click="closeModalWithAnimation" 
                  class="group relative px-6 py-2.5 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors duration-200 overflow-hidden"
                >
                ...
                <button 
                  @click="saveCategory" 
                  class="group relative px-6 py-2.5 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200 overflow-hidden"
                >
            ...
            >
              <div class="text-center">
                ...
                <div class="flex justify-center space-x-3">
                  ...
                  <button 
                    @click="deleteWithAnimation" 
                    class="group relative px-6 py-2.5 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-lg shadow-md hover:shadow-lg transition-all duration-200 overflow-hidden"
                  >
      ...
      <Transition name="toast">
        <div 
          v-if="toast.show" 
          class="fixed bottom-4 right-4 z-50 max-w-md bg-white rounded-lg shadow-lg border-l-4 p-4 transform transition-all duration-500"
          :class="{
  ...
  // Composable for category management
  const useCategoryStore = () => {
    const categories = ref<Category[]>([
      { id: 1, name: 'Rice', menuCount: 3 },
      { id: 2, name: 'Moi-Moi & Salad', menuCount: 2 },
      { id: 3, name: 'Shawarma and Pizza', menuCount: 2 },
      { id: 4, name: 'Meat & Fish', menuCount: 8 },
      { id: 5, name: 'Porridge', menuCount: 1 },
      { id: 6, name: 'Cakes', menuCount: 7 },
      { id: 7, name: 'Meat & Chicken Pie', menuCount: 2 },
      { id: 8, name: 'Toast Bread', menuCount: 2 },
      { id: 9, name: 'Grillz', menuCount: 2 },
      { id: 10, name: 'Cocktails & Mocktails', menuCount: 2 }
    ])
    ...
    const addCategory = (name: string) => {
      ...
      return { id: newId, name, menuCount: 0 }
    }
  ...
  const saveCategory = () => {
    ...
    if (isEditing.value) {
      const updated = updateCategory(categoryForm.value.id, categoryForm.value.name)
      if (updated) {
        showToast('Success', `Category "${updated.name}" has been updated`, 'success')
      }
    } else {
      const added = addCategory(categoryForm.value.name)
      showToast('Success', `Category "${added.name}" has been created`, 'success')
    }
  ...
  const deleteWithAnimation = () => {
    if (categoryToDelete.value) {
      const deleted = removeCategory(categoryToDelete.value.id)
      if (deleted) {
        showToast('Deleted', `Category "${deleted.name}" has been removed`, 'info')
      }
  ...
  onMounted(() => {
    // Simulate loading for content animation
    setTimeout(() => {
    ...
    if (categoryInput.value) {
      categoryInput.value.addEventListener('focus', handleInputFocus)
      categoryInput.value.addEventListener('blur', handleInputBlur)
    }
  })
